...
public class ArmorDurabilityMod implements ClientModInitializer {
    ...
    private void renderArmorHud(DrawContext context, float tickDelta) {
        ...
        int x = 10; // top-left HUD position
        int y = 10;

        // Background
        RenderSystem.enableBlend();
        int bgColor = 0x90000000; // translucent black
    ...
    private Text formatArmorLine(String label, ItemStack stack) {
        ...
        int color;
        if (percent >= 60)      color = 0x55FF55;  // green
        else if (percent >=30) color = 0xFFFF55;  // yellow
        else                   color = 0xFF5555;  // red
